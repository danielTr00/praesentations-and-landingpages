/**
 * CLG Vermögenenschutz - Contact Form JavaScript
 * Handles form submission, validation, error handling, and API communication.
 * Supports both localhost and SSLIP.io testing environments with HTTPS.
 */
...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', async function(e) {
            ...
            if (!isValidEmail(email)) {
                setStatus(statusDiv, 'Bitte geben Sie eine gültige E-Mail-Adresse ein.', 'error');
                resetButton(submitBtn);
                return;
            }
            ...
            try {
                ...
                if (data && data.status === 'success') {
                    ...
                    setStatus(statusDiv, 'Ein Fehler ist aufgetreten. Bitte versuchen Sie es später erneut.', 'error');
                }
            } catch (error) {
    ...
    function detectApiBaseUrl() {
        ...
        // If running on SSLIP domain, use HTTPS for API
        if (hostname.includes('.sslip.io')) {
            const ipMatch = hostname.match(/^(.+)\.sslip\.io$/);
            if (ipMatch) {
                return `https://${ipMatch[1]}:8443`;  // HTTPS on port 8443
            }
        }
